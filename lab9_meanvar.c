...
start = offset*VECLEN/NUMTHRDS;
end = start + VECLEN/NUMTHRDS;
...
//I didn't have time to create a new function to compute the variance so the code below of 
//squaring the total sum is incorrect but prints something at least




double y = (vector1.sum*vector1.sum)-(vector1.mean*vector1.mean);
 vector1.var = (y)/(vector1.size-1);
printf("Variance =  %f \n", vector1.var);
/*     PHASE 3        */
